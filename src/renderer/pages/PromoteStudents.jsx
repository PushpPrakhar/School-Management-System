...
function StepYear({ onNext }) {
  ...
  const handleFromChange = (val) => {
    ...
    if (idx !== -1 && idx < YEAR_OPTIONS.length - 1) setToYear(YEAR_OPTIONS[idx + 1]);
  };

  return (
    <div className="max-w-xl">
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 mb-6">
        <p className="text-sm font-semibold text-amber-800 mb-1">⚠️ Year-end operation</p>
        <p className="text-xs text-amber-700">
          Run this once at the end of the academic year. Every active student moves up
          one class. Class 12 students will be marked Passed Out.
          Take a database backup before proceeding.
        </p>
      </div>
      <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-5">
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">Promoting FROM</label>
          <select value={fromYear} onChange={e => handleFromChange(e.target.value)}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
            {YEAR_OPTIONS.slice(0, -1).map(y => <option key={y}>{y}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">Promoting TO</label>
          <select value={toYear} onChange={e => setToYear(e.target.value)}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
            {YEAR_OPTIONS.slice(1).map(y => <option key={y}>{y}</option>)}
          </select>
        </div>
        <div className="bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 text-sm text-gray-600">
          Students will move from <strong>{fromYear}</strong> → <strong>{toYear}</strong>
        </div>
        <button onClick={() => onNext(fromYear, toYear)}
...
function StepPreview({ fromYear, toYear, onBack, onConfirm }) {
  ...
  const toggleStudent = (admNo) => setExcluded(prev => {
  ...
  const toggleClass = (students) => {
    ...
    setExcluded(prev => {
      const next = new Set(prev);
      students.forEach(s => allOut ? next.delete(s.admission_number) : next.add(s.admission_number));
      return next;
    });
  };
  ...
  const passedOut = preview?.preview
    ?.filter(p => p.next_class === 'PASSED OUT')
    .reduce((s, p) => s + p.students.filter(st => !excluded.has(st.admission_number)).length, 0) || 0;
  ...
  return (
    <div>
      ...
      <p className="text-xs text-gray-400 bg-gray-50 border border-gray-200 rounded-lg px-4 py-2 mb-4">
        💡 Uncheck individual students to exclude them from promotion — excluded students stay in their current class.
      </p>

      <div className="space-y-2">
        {preview?.preview?.map(p => {
          const allOut     = p.students.every(s => excluded.has(s.admission_number));
          const someOut    = !allOut && p.students.some(s => excluded.has(s.admission_number));
          const isExpanded = expanded === p.current_class;
          return (
            <div key={p.current_class} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              <div className="flex items-center gap-4 px-5 py-4 bg-gray-50">
                <input type="checkbox" checked={!allOut}
                  ref={el => { if (el) el.indeterminate = someOut; }}
                  onChange={() => toggleClass(p.students)}
                  className="w-4 h-4 accent-blue-600" />
                <div className="flex-1 flex items-center gap-3">
                  <span className="font-semibold text-gray-800">{p.current_class}</span>
                  <span className="text-gray-400">→</span>
                  <span className={`font-semibold text-sm ${p.next_class === 'PASSED OUT' ? 'text-green-600' : 'text-blue-700'}`}>
                    {p.next_class || '⚠️ Unknown'}
                  </span>
                </div>
                <span className="text-xs text-gray-400">
                  {p.students.filter(s => !excluded.has(s.admission_number)).length} / {p.count}
                </span>
                <button onClick={() => setExpanded(isExpanded ? null : p.current_class)}
              ...
              {isExpanded && (
                <div className="max-h-56 overflow-y-auto divide-y divide-gray-50">
                  {p.students.map(s => (
                    <label key={s.admission_number}
                      className={`flex items-center gap-3 px-5 py-2 cursor-pointer hover:bg-gray-50 ${excluded.has(s.admission_number) ? 'bg-red-50' : ''}`}>
                      <input type="checkbox" checked={!excluded.has(s.admission_number)}
                        onChange={() => toggleStudent(s.admission_number)}
                        className="w-4 h-4 accent-blue-600" />
                      <span className={`text-sm flex-1 ${excluded.has(s.admission_number) ? 'line-through text-gray-400' : 'text-gray-700'}`}>
                        {s.student_name}
                      </span>
                      <span className="text-xs font-mono text-gray-400">{s.admission_number}</span>
                      {excluded.has(s.admission_number) && (
                        <span className="text-xs bg-red-100 text-red-500 px-2 py-0.5 rounded-full">Excluded</span>
                      )}
                    </label>
                  ))}
                </div>
      ...
      <div className="flex items-center justify-between mt-6 pb-4">
        ...
        <button onClick={() => onConfirm(Array.from(excluded))} disabled={toPromote === 0}
...
function StepConfirm({ toYear, excluded, onBack, onDone }) {
  ...
  if (result) return (
    <div className="max-w-lg mx-auto text-center py-8">
      ...
      <h3 className="text-xl font-bold text-gray-800 mb-4">Promotion Complete!</h3>
    ...
    <div className="max-w-lg">
      <div className="bg-red-50 border border-red-200 rounded-xl p-5 mb-6">
        ...
        <p className="text-sm text-red-600">
          This permanently updates <strong>current_class</strong> for all selected students.
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 mb-5">
        <label className="flex items-start gap-3 cursor-pointer">
          ...
          <span className="text-sm text-gray-700">
            I have taken a database backup and confirm promoting all selected students to
            the next class for academic year <strong>{toYear}</strong>.
          </span>
        </label>
      </div>
      ...
      <div className="flex items-center justify-between">
        <button onClick={onBack} className="text-sm text-gray-400 hover:text-gray-600 underline">← Back</button>
        <button onClick={execute} disabled={!confirmed || saving}
          ...
          {saving ? <><span className="animate-spin">⏳</span> Promoting…</> : '✅ Confirm & Promote'}
        </button>
      </div>
    </div>
  );
...
function HistoryTab() {
  ...
  return (
    <div className="space-y-3">
      {history.map((h, i) => (
        <div key={i} className="bg-white border border-gray-200 rounded-xl px-5 py-4 flex items-center justify-between">
          <div>
            <p className="font-semibold text-gray-800">{h.changes?.[0]?.new || 'Promotion'}</p>
            <p className="text-xs text-gray-400 mt-0.5">By {h.edited_by} · {h.edited_at?.slice(0,16)}</p>
          </div>
          <span className="text-xs bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded-full">Completed</span>
        </div>
      ))}
    </div>
  );
...
export default function PromoteStudents() {
  ...
  const reset = () => { setStep(1); setExcluded([]); };
  ...
  return (
    <div className="max-w-4xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-0.5">Move all active students to the next class at year end</p>
      </div>

      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl mb-6 w-fit">
        {[['promote','Run Promotion'],['history','Promotion History']].map(([key, label]) => (
      ...
      {tab === 'promote' && (
        <>
              ...
              return (
                <React.Fragment key={n}>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm ${step === n ? 'font-semibold text-blue-700' : 'text-gray-400'}`}>{label}</span>
                  </div>
                  {i < STEPS.length - 1 && (
                    <div className={`flex-1 h-px mx-3 ${step > n ? 'bg-green-500' : 'bg-gray-200'}`} />
                  )}
                </React.Fragment>
              );
            })}
          ...
          {step === 1 && <StepYear onNext={(from, to) => { setFromYear(from); setToYear(to); setStep(2); }} />}
          {step === 2 && <StepPreview fromYear={fromYear} toYear={toYear} onBack={() => setStep(1)} onConfirm={excl => { setExcluded(excl); setStep(3); }} />}
          {step === 3 && <StepConfirm toYear={toYear} excluded={excluded} onBack={() => setStep(2)} onDone={reset} />}
        </>
      )}
    </div>
  );
