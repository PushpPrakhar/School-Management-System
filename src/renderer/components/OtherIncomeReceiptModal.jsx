...
export default function OtherIncomeReceiptModal({ receiptNumber, academicYear, onClose }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const res = await window.api.otherIncomeGetReceiptPrintData(receiptNumber, academicYear);
      if (cancelled) return;
      setLoading(false);
...
function ReceiptContent({ data }) {
  const { students, totals, item_names } = data;
  const modeLabel = MODE_LABEL[data.payment_mode] || data.payment_mode;

  return (
    <div className="print-root border border-gray-300 p-6 text-sm print:border-none" id="other-income-receipt-print">
      {/* Letterhead */}
      <div className="text-center border-b-2 border-gray-800 pb-3 mb-4">
        ...
        <h2 className="text-lg font-bold mt-2 tracking-wide">OTHER CHARGES RECEIPT</h2>
      </div>
      ...
      {/* Students table — columns are whichever items appear in this receipt */}
      <table className="w-full text-[11px] border border-gray-400 border-collapse mb-3">
        <thead>
          <tr className="bg-gray-100 text-center">
            <th className="border border-gray-400 px-1 py-1">Student Ledger No</th>
            <th className="border border-gray-400 px-1 py-1">Student's Name &amp; Class</th>
            {item_names.map(name => <th key={name} className="border border-gray-400 px-1 py-1">{name}</th>)}
            <th className="border border-gray-400 px-1 py-1">Total</th>
            <th className="border border-gray-400 px-1 py-1">Concession</th>
        ...
        <tbody>
          {students.map(s => (
            <tr key={s.admission_number}>
              <td className="border border-gray-400 px-1 py-1 font-semibold text-blue-700">{s.sl_number || s.admission_number}</td>
              <td className="border border-gray-400 px-1 py-1">
                {s.student_name}
                <br /><span className="text-gray-500">{s.current_class}{s.section ? ' ' + s.section : ''}</span>
              </td>
              {item_names.map(name => (
                <td key={name} className="border border-gray-400 px-1 py-1 text-right">{fmt(s.items[name] || 0)}</td>
              ))}
              <td className="border border-gray-400 px-1 py-1 text-right font-semibold">{fmt(s.total_amount)}</td>
              <td className="border border-gray-400 px-1 py-1 text-right">{fmt(s.total_concession)}</td>
              <td className="border border-gray-400 px-1 py-1 text-right">{fmt(s.total_paid)}</td>
            </tr>
          ))}
          <tr className="bg-gray-50 font-bold">
            <td className="border border-gray-400 px-1 py-1" colSpan={2}>Total</td>
            {item_names.map(name => (
              <td key={name} className="border border-gray-400 px-1 py-1 text-right">
                {fmt(students.reduce((s, st) => s + (st.items[name] || 0), 0))}
              </td>
            ))}
            <td className="border border-gray-400 px-1 py-1 text-right">{fmt(totals.total_amount)}</td>
            <td className="border border-gray-400 px-1 py-1 text-right">{fmt(totals.total_concession)}</td>
            <td className="border border-gray-400 px-1 py-1 text-right">{fmt(totals.total_paid)}</td>
          </tr>
        </tbody>
      </table>
